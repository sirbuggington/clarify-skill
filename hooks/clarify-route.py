...
INJECTION_TEXT = (
    ...
    "advice, diagnosis, choice, or \"next steps,\" AND any user-unique "
    "fact (situation, goal, history, constraints, preferences) could "
    "materially change it, your FIRST action MUST be `AskUserQuestion`. "
    "Answering first and acknowledging the gap is a protocol violation, "
    "not a graceful answer.\n\n"
    "**Gate:**\n"
    "1. **Tool can resolve?** Read files, search, check context. If a tool "
    "can answer, use it. If not, go to 2.\n"
    ...
    "minimum; skip intrusive details. **One sentence per question; short "
    "option labels; full detail only when meaning requires it.**\n\n"
